...
document.writeln("	<div id=\"title\" class=\"title\">我要领奖<span class=\"loginclose\" id=\"loginclose\"><\/span><\/div>");
document.writeln("	<div class=\"content\">");
document.writeln("	<div class=\"username\">");
document.writeln("	您当前排名：" + mypos + "名");
document.writeln("	 <\/div>");
document.writeln("	<div class=\"userphone\">");
document.writeln("	获得奖品：" + mygift);
document.writeln("	 <\/div>");
document.writeln("	<div class=\"userphone\">");
document.writeln("	兑换门店：" + buildShop());
document.writeln("	 <\/div>");
document.writeln("	<div class=\"userphone shopmsg\" style=\"color:red;\" >");
document.writeln("	 <\/div>");

document.writeln("	<span id=\"err_msg\" style=\"color:red\"><\/span>");
document.writeln("	 <input type=\"button\" value=\"申请兑奖\" id=\"loginbutton\" name=\"申请兑奖\" class=\"txtbtn\">	");
document.writeln("	<\/div>");
document.writeln("<\/div>");
...
function chkshop(obj) {
	//alert('dddd')
	//alert($('#selshop').find("option:selected").attr('addr'));
	if (obj.value == '-1' || obj.value == '其它门店') {
		$('.shopmsg').html(cfg_other_shop);
		
	} else if (obj.value == '') {
		$('.shopmsg').html('');
	} else {
		$('.shopmsg').html($('#selshop').find("option:selected").attr('addr'));
	}
...
function buildShop() {
	...
	//alert(shopList);
	shopArr = eval('(' + shopList +')');
	shopStr = '<select name="selshop" id="selshop" onchange="chkshop(this);" >'
	shopStr += '<option value="">请选择</option>';
	for (var i = 0; i < shopArr.length; i++) {
		shopStr += '<option value="' + shopArr[i].name + '" addr="' + shopArr[i].addr + '" >' + shopArr[i].name + '</option>';
	}
	
	shopStr += '<option value="其它门店">其它门店</option>';
	shopStr += '</select>';	
	return shopStr;
}
...
$("#loginbutton").click(function () { 
	...
	if(err_msg!=''){
		...
		url : './doDuijiang',
		data : {
				"selshop" : encodeURIComponent(selshop),
				"giftname":encodeURIComponent(mygift)
			},
		type : 'post',
		dataType : "json",
		async : false,
