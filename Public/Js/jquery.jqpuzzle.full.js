...
var sound = new Audio('/Public/Images/move.mp3');
var gamecoin = 0;
sound.load();
...
$.fn.jqPuzzle = function(settings, texts) {
	...
	var defaults = {
		...
		control: {
			...
			pauseGame:true,	// display 'Pause' button [true|false] 
			continueGame:true,	// display 'Continue' button [true|false] 
			replayGame:true, 	// display 'Replay' button [true|false] 
	...
	var i18n = {
		en: {
			...
			pauseGameLabel:			'PauseGame',
			continueGameLabel:		'ContinueGame',
			replayGameLabel:		'ReplayGame',
...
//			var imgWidth =document.body.clientWidth * 0.8;
//			var imgHeight = imgWidth * 240 / 320 ;
			...
			var getOffset = function(row, col) {
				var offset = {
					left: (computedStyles.wrapper.padding + col*totalPieceWidth)*style.zoom,
					top:  (computedStyles.wrapper.padding + row*totalPieceHeight)*style.zoom
				};
			...
			var shuffle = function(rounds, speed) {
				...
				if(speed) {
					...
					if($shuffleButton.is('.jqp-disabled')) {
				...
				if($timer) { $timer.val(seconds); $("#game_counter").text(seconds);playerRecord = seconds;  }
			...
			// create elements -------------------------------------------------
//			var $mytimer = ('<span id="game_counter_line" style="text-align: right">用时：<span id="game_counter">0</span>s</span>');
			...
			var $wrapper = $('<div/>')
				...
				.css({	
					width: (fullWidth + boxModelHack.wrapper) * style.zoom,
					height: (fullHeight + boxModelHack.wrapper) * style.zoom,
					borderWidth: (computedStyles.wrapper.border)*style.zoom,
					padding: (computedStyles.wrapper.padding)*style.zoom,
					margin: 0,
					position: 'relative',
					overflow: 'hidden',
					display: 'block',
			...
			var $protoPiece = $('<div/>')
				.addClass('jqp-piece')
				.css({
					width: (width + boxModelHack.piece)*style.zoom,
					height: (height + boxModelHack.piece)*style.zoom,
					backgroundImage: 'url(' + imgSrc + ')',
					borderWidth: (computedStyles.piece.border)*style.zoom,
					margin: 0,
					padding: 0,
					position: 'absolute',
					overflow: 'hidden',
			...
			var $protoButton = $('<span/>').css('cursor', 'default');
			lock = true;
			...
			// create pause button
			if(control.pauseGame) {
				$pauseGameButton = $protoButton.clone()
					.text(texts.pauseGameLabel)
					.attr('id','pauseGameButton')
					.appendTo($controls);
			...
			if(control.continueGame) {
				$continueGameButton = $protoButton.clone()
					.text(texts.continueGameLabel)
					.attr('id','continueGameButton')
					.appendTo($controls);
			...
			if(control.replayGame) {
				$replayGameButton = $protoButton.clone()
					.text(texts.replayGameLabel)
					.attr('id','replayGameButton')
					.appendTo($controls);
			...
			// pause game  on click
			if(control.pauseGame) $pauseGameButton.click(function() {
			...
			if(control.continueGame) $continueGameButton.click(function() {
				...
				if($originalButton.is('.jqp-toggle')) {
					...
					if(control.pauseGame) $pauseGameButton.removeClass('jqp-disabled');
					if(control.continueGame) $continueGameButton.removeClass('jqp-disabled');
					...
					$background.fadeTo(animation.fadeOriginalSpeed, style.backgroundOpacity, function() {
						...
						// resume timer
						//if(control.continueGame && timer) timer.resume();
			...
			if(control.replayGame) $replayGameButton.click(function() {
			...
			if(control.toggleOriginal) $originalButton.click(function() {
				...
				lock = true; // set lock
					...
					if(control.pauseGame) $pauseGameButton.addClass('jqp-disabled');
					if(control.continueGame) $continueGameButton.addClass('jqp-disabled');
